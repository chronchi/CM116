...
random_vector = [sample(1:50,10,replace=false); sample(51:100,10,replace=false);
...
#Função sigmoide
sigm(z) = 1/(1+exp(-z))
#Ponto inicial para os métodos do gradiente e gradiente acelerado
...
for i = 1:3
  ...
  thetai, iter = metgradnest(grad,thetarand,L,max_iter=50000)
  parameters[:,i] = thetai
  iterlistnest = [iterlistnest; iter]
end
...
for k = 1:3
  ...
  println("O modelo para o $k-ésimo parâmetro apresenta uma taxa de acerto
  de $(100*count/30)%")
end
